...
def migrate(api_source, api_target, workspaces_map, teams_map):
    ...
    for source_workspace_id in workspaces_map:
        ...
        source_workspace_teams = api_source.team_access.list(filters=source_workspace_team_filters)["data"]
        ...
        target_workspace_teams = api_target.team_access.list(filters=target_workspace_team_filters)["data"]
        ...
        for source_workspace_team in source_workspace_teams:
            ...
            if new_target_team_id in target_team_ids:
                print("\t workspace %s team access for team %s exists, skipping..." % \
                    (target_workspace_id, new_target_team_id))
                continue
...
def delete_all(api_target):
    print("Deleting team workspace access...")
        ...
        for target_workspace in target_workspaces:
            ...
            if target_workspace_teams:
                for target_workspace_team in target_workspace_teams:
                    target_workspace_team_data = api_target.teams.show( \
                        target_workspace_team["relationships"]["team"]["data"]["id"])["data"]
                    target_workspace_team_name = target_workspace_team_data["attributes"]["name"]

                    print(f"\t deleting %s team access from workspace %s..." \
                        % (target_workspace_team_name, target_workspace["attributes"]["name"]))
                    api_target.team_access.remove_team_access(target_workspace_team["id"])

    print("Team workspace access deleted.")